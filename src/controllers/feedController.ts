...
export async function getFeedFromFollowing(req: Request, res: Response, next: NextFunction) {
	...
	try {
		const response: IFeedResponse = await fromFollowing(
			userProfileModel.getFollowingListByUserId.bind(userProfileModel),
			tweetModel.getFollowingFeedForUser.bind(tweetModel),
			userProfile._id,
		);
		return res.status(response.error ? 400 : 200).send(response);
	} catch (err) {
